...
const StarRating = ({ rating }) => {
  ...
  return (
    <div className="flex items-center gap-1">
      {[1, 2, 3, 4, 5].map((star) => (
        <svg key={star} className={`size-3.5 ${value > 0 && star <= Math.round(value) ? "text-yellow-400" : "text-gray-300"}`} fill="currentColor" viewBox="0 0 20 20">
                  ...
                  <div className="flex items-center gap-2 mt-0.5">
                    ...
                    <span className="text-xs text-gray-500">
                      {clientStats.ratingCount > 0
                        ? `(${clientStats.ratingCount})${clientStats.completionRate != null ? ` · ${Math.round(clientStats.completionRate * 100)}% completados` : ""}`
                        : "sin calificar aún"}
                    </span>
                  </div>
                )}
